/* ============================================================
   ...
   una vez publicado (ver Code.gs). Si se deja vacío, la página
   funciona igual pero con los datos de ejemplo de más abajo.
   ============================================================ */
...
/* ============================================================
   ...
   (sirve como valor de arranque / respaldo si el backend falla)
   ============================================================ */
const NEGOCIO = {
...
/* ---------------- Carga de datos remotos (Google Sheets) ---------------- */
async function cargarDatosRemotos(){
  ...
  try{
    ...
    if(data.servicios && data.servicios.length){
      NEGOCIO.servicios = data.servicios.filter(s => s.activo);
    }
    if(data.config){
      NEGOCIO.diasAtencion = data.config.diasAtencion;
      NEGOCIO.horaApertura = data.config.horaApertura;
      NEGOCIO.horaCierre = data.config.horaCierre;
      NEGOCIO.duracionSlotMin = data.config.duracionSlot;
      NEGOCIO.descansos = data.config.descansos;
    }
    if(data.turnos){
      NEGOCIO.turnosOcupados = data.turnos.filter(t => t.estado !== 'cancelado');
    }
    renderEstatico();
    renderServiciosList();
    if(typeof renderDays === 'function') renderDays();
  } catch(err){
    console.error('No se pudo conectar con el backend, se usan los datos de ejemplo.', err);
  }
}
...
async function confirmarWhatsApp(){
  ...
  if(API_URL){
    try{
      ...
      console.error('No se pudo registrar el turno en el backend, se continúa igual por WhatsApp.', err);
    }
  }
